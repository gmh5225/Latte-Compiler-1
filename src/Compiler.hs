...
compileFuncDef (FnDef _ retType (Ident name) args block) = do
  ...
  newState <- get
  -- TODO:
  put newState {sPenv = sPenv prevState, sVenv = sVenv prevState, sStore = sStore prevState}
...
compileStmt (BStmt _ (Block _ stmts)) = do
  ...
  newState <- get
  put state {sStore = sStore newState, sRegister = sRegister newState, sLabel = sLabel newState }
...
compileStmt (Ret pos expr) = do
  ...
  case val of
    ...
    (RegV exprReg) -> return (code ++ show (RetI exprType exprReg), strDeclarations)
    (BoolV boolVal) -> return (code ++ "ret i1 "++ show (BoolV boolVal) ++ "\n" , strDeclarations)
...
compileStmt (CondElse _ expr stmt1 stmt2) = do
  ...
  resultCode <- generatePhi (sStore prevState) (sStore stateAfterTrueBlock) (sStore stateAfterFalseBlock) endTrue endFalse
...
compileStmt (While pos expr stmt) = do
  ...
  condPhi <- generate3Phi (sStore updatedState) (sStore prevState) (sStore stateAfterBodyCompl) labBase endBody


  let result = show (JmpI labBase) ++
         show labBase ++ ":\n" ++
        show (JmpI labCond) ++
        show labCond ++ ":\n" ++ 
        condPhi ++ 
...
complieExpression (Not pos expr) = do
  ...
  reg <- useNewReg
  case exprResult of
    BoolV False -> do return (BoolV True, code, CBool, strDeclarations)
    BoolV True -> do return (BoolV False, code, CBool, strDeclarations)
    IntV exprVal -> do return (IntV exprVal, code, CBool, strDeclarations)
...
complieAndOr f pos e1 e2 = do
  ...
  (code,sd) <- compileStmt (if f == "or" then --type insted of string?
    (Cond pos (Not pos (EVar pos ident)) (Ass pos ident e2)) else
    (Cond pos (EVar pos ident) (Ass pos ident e2))) 
